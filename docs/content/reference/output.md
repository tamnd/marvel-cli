...
describes what users get. Pick a format with `-o`, or let marvel choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
marvel <command> -o table   # aligned columns for reading
marvel <command> -o jsonl   # one JSON object per line, for piping
marvel <command> -o json    # a single JSON array
marvel <command> -o csv     # spreadsheet friendly
marvel <command> -o tsv     # tab-separated
marvel <command> -o url     # just the URL column
marvel <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
marvel <command> --fields id,title,url
```
...
```bash
marvel <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
marvel <command>            # a table, because this is a terminal
marvel <command> | wc -l    # JSONL, because this is a pipe
```
